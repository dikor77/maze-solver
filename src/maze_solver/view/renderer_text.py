...
def Cell_string(square: Square, col_width: int = 7, row_height: int = 3) -> list[str]:
    buffer = list()
    top = ' '*(col_width-2)
    bot = ' '*(col_width-2)
    left = ' '
    ...
    bot_right = ' '
    body = ' '*(col_width-2)
    if Border.TOP in square.border:
        top = '─'*(col_width-2)
        top_left = top_right = '─'
    if Border.BOTTOM in square.border:
        bot = '─'*(col_width-2)
        bot_left = bot_right = '─'
    if Border.LEFT in square.border:
        left = top_left = bot_left = '│'
    if Border.RIGHT in square.border:
        right = top_right = bot_right = '│'
    if Border.TOP|Border.LEFT in square.border:
        top_left = '┌'
    if Border.TOP|Border.RIGHT in square.border:
        top_right = '┐'
    if Border.BOTTOM|Border.LEFT in square.border:
        bot_left = '└'
    if Border.BOTTOM|Border.RIGHT in square.border:
        bot_right = '┘'
    
    r=str(square.role.name)[:3]
    r= "YYY" if r == "EXT" else r
    r= "XXX" if r == "WAL" else r
    r= "   " if r == "NON" else r

    body = r.center(len(body), ' ')

    buffer.append(str(top_left + top + top_right))
    for _ in range(row_height-2):
        buffer.append(str(left + body + right))
    buffer.append(str(bot_left + bot + bot_right))

    return buffer


def Maze_string(maze: Maze, col_width: int = 7, row_height: int = 3) -> list[str]:
    buffer = ['*'*(maze.width*col_width)]*(maze.height*row_height)
    for row in range(maze.height):
        for col in range(maze.width):
            idx = row * maze.width + col
            square = maze[idx]
            cell_strings = Cell_string(square)

            col_buffer = col * col_width
            row_buffer = row * row_height
            for line in cell_strings:
                num = len(line)
                buffer[row_buffer] = buffer[row_buffer][:col_buffer] + line + buffer[row_buffer][col_buffer+num:]
                row_buffer += 1

    return buffer


def Add_Solution(solution:Solution, buffer: list[str], col_width: int = 7, row_height: int = 3) -> list[str]:
    for cell in solution:
        col_buffer = cell.column * col_width + 1
        row_buffer = cell.row * row_height
        sym = ':'*(col_width-2)
        num = len(sym)
        for i in range(row_height):
            if(i>0 and i<(row_height-1)):
                buffer[row_buffer] = buffer[row_buffer][:col_buffer] + sym + buffer[row_buffer][col_buffer+num:]
            row_buffer += 1
    return buffer
...
def test():
    ...
    maze_text = Maze_string(maze)

    solutions = solve_all(maze)
    if solutions:
        Add_Solution(solutions[0], maze_text)
    else:
